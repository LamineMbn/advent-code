...
private val input = readFileLineByLineUsingForEachLine("src/day10/input_AoC_input_1.txt").map { it.toCharArray() }
...
fun main() {
    ...
    while (mutableTarget.size < numberOfAsteroids){
        for (ast in mergeMap.keys){
            ...
            val asteroidsByAngle = mergeMap.getValue(ast).sortedBy { radius((it.x - overlookPosition.x).toDouble(), (it.y - overlookPosition.y).toDouble()) }
