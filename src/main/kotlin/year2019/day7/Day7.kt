...
//private val amplifierControllerSoftware: List<Long> = main.kotlin.readFileLineByLineUsingForEachLine("src/main/kotlin/2019/day7/input_AoC_input_1.txt").flatMap{ it.split(",") }.map { it.toLong() }
...
fun main() {
    ...
    for (perm in settings) {
        for (case in perm) {
            amplifiers.plusAssign(
                Amplifier(
                    amplifierControllerSoftware.toMutableList(),
                    case.toLong()
                )
            )
        }
