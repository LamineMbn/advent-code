...
//    println(intersection)
    ...
    var manhattanDistance: List<Int> = ArrayList()

    for (intersect in intersection) {
        if (intersect != Pair(0, 0)) {
            manhattanDistance += calculateManhattanDistance(intersect)
        }
    }

    println(manhattanDistance.min())

}

private fun retrievePairs(paths: List<String>): List<Pair<Int, Int>> {
    var x1 = 0
    var y1 = 0
    var pathIndexes: List<Pair<Int, Int>> = ArrayList()

    for (path in paths) {
        println("Working on $path....")
        val direction = retrieveDirection(path)
        val step = retrieveSteps(path)

        if (direction == "R") {
            for (i in 1..step) {
                pathIndexes += Pair(y1, i + x1)
            }
            x1 += step
        }
...
private fun calculateManhattanDistance(point: Pair<Int, Int>): Int {
    return abs(point.first) + abs(point.second)
}

private fun retrieveDirection(path: String): String {
    return path.substring(0, 1)
}

private fun retrieveSteps(path: String): Int {
