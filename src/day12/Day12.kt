...
fun main() {
    ...
    var ioPos = Moon(Coordinate(17, -12, 13),Coordinate(0, 0, 0))
    var europaPos = Moon(Coordinate(2, 1, 1),Coordinate(0, 0, 0))
    var ganymedePos = Moon(Coordinate(-1, -17, 7),Coordinate(0, 0, 0))
    var callistoPos = Moon(Coordinate(12, -14, 18),Coordinate(0, 0, 0))
        ...
        if (step > 0) {
            for (i in 0 until moons.size) {
                ...
                for (j in i + 1 until moons.size) {
                    ...
                    val offset = retrieveVelocityOffset(currentMoon.position, moonToCompareTo.position)
                    currentMoon.velocity = updateVelocityOfCurent(currentMoon, offset)
                    moonToCompareTo.velocity = updateVelocityOfComparable(moonToCompareTo, offset)
                }
            }
        }
        moons.forEach {
...
//    val previousStates = State(ioPos.position, europaPos.position, ganymedePos.position, callistoPos.position)
    ...
    val previousPoXState = mutableListOf(createState(retrieveXPosition(moons)))
    val previousVelocityXState = mutableListOf(createState(retrieveXVelocity(moons)))

    val previousPoYState = mutableListOf(createState(retrieveYPosition(moons)))
    val previousVelocityYState = mutableListOf(createState(retrieveYVelocity(moons)))

    val previousPoZState = mutableListOf(createState(retrieveZPosition(moons)))
    val previousVelocityZState = mutableListOf(createState(retrieveZVelocity(moons)))
...
//        println("After ${stepNeeded} step")
        ...
        val currentPositionXState = createState(retrieveXPosition(moons))
        val currentVelocityXState = createState(retrieveXVelocity(moons))

        val currentPositionYState = createState(retrieveYPosition(moons))
        val currentVelocityYState = createState(retrieveYVelocity(moons))

        val currentPositionZState = createState(retrieveZPosition(moons))
        val currentVelocityZState = createState(retrieveZVelocity(moons))
...
//            previousVelocityZState.plusAssign(currentVelocityZState)
    ...
    println(calcLeastCommonMultiple(stepNeededX.toLong(),stepNeededY.toLong(),stepNeededZ.toLong()))
    println("Steps Needed ${calculateMiddleCycle(listOf(stepNeededX,stepNeededY,stepNeededZ))}")
...
private fun createState(list : MutableList<Int>) = State(list[0], list[1], list[2], list[3])
...
private fun updateVelocityOfCurent(currentMoon: Moon, offset: Coordinate) =
    Coordinate(currentMoon.velocity.x + offset.x, currentMoon.velocity.y + offset.y, currentMoon.velocity.z + offset.z)
